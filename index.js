...
const runApp = () => {
    ...
    function renderPatients() {
        ...
        [...patients].sort((a,b) => {
            ...
            const patientAppointments = appointments
                .filter(a => p && a.patientId === p.id && a.date <= todayStr)
    ...
    function renderReport() {
        ...
        const todayMonthBtn = document.getElementById('goto-current-month-btn');
                    ...
                    <div class="space-y-1">
                        ${['ASA', 'OSS', 'CONSULENZA', 'EXTRA'].map(cat => `
                            <div class="flex justify-between">
                                <span>${cat}</span>
                                <span class="font-semibold">${(totalsByCategory[cat] || 0).toFixed(2)} ore</span>
                            </div>
                        `).join('')}
                    </div>
    ...
    appointmentForm.addEventListener('submit', (e) => {
        ...
        if (id) {
            ...
            const newAppointment = { ...appointmentData, id: Date.now().toString() + Math.random().toString() };
